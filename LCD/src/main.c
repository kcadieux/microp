...
const char *openingText[3];

void initOpening()
{
	openingText[0] = " A long time ago in ";
	openingText[1] = "  a galaxy far far  ";
	openingText[2] = "       away...      ";
}
...
void lcdDisplay(void const *argument)
{
	...
	for (xpos = 150/*LCD_PIXEL_HEIGHT*/; xpos>0 /* -(LCD_GetFont()->Height * numberOfLines)*/; xpos--)
	{
		for (line = 0; line < numberOfLines; line++)
		{
			//LCD_DisplayStringLine(xpos+1+(line*LCD_GetFont()->Height), (uint8_t*)openingText[line]);
			
			//LCD_SetColors(LCD_COLOR_BLACK, LCD_COLOR_BLACK);
			//LCD_DrawFullRect(0, xpos + (line*LCD_GetFont()->Height), LCD_PIXEL_WIDTH, LCD_GetFont()->Height+2);
			//LCD_DrawLine(0, xpos + 3+ ((line + 1)*(LCD_GetFont()->Height+2)), LCD_PIXEL_WIDTH, LCD_DIR_HORIZONTAL);
			//LCD_SetColors(LCD_COLOR_YELLOW, LCD_COLOR_BLACK);
			//LCD_Clear(LCD_COLOR_BLACK);
			
			LCD_DisplayStringLine(xpos + (line*(LCD_GetFont()->Height+2)), (uint8_t*)openingText[line]);
			osDelay(8);
		}
		LCD_Clear(LCD_COLOR_BLACK);
	}
